...
|当前回归证据|本机 `run_regression.py`：50 个脚本通过；GitHub Actions macOS/Windows × Python 3.10/3.13 四矩阵通过；`quality_gate.py --privacy` 通过；新增跨项目、失败 ledger、真实 PreToolUse scope hook、事务、施工协议、状态迁移和 200 次并发测试已纳入回归|
...
不得先把“计划”写成“已实现”。专题入口在施工中保持“实施中”，全部门槛通过后才改“已完成”。当前本机回归已扩展至 50 个脚本，GitHub Actions 四矩阵已通过；真实跨项目只读 smoke、完整 coverage evidence 和 8 个 WP 的 accepted packet 仍未形成可发布证据。
