...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="construction-protocol-") as directory:
        ...
        (root / "review-input.json").write_text(json.dumps(review), encoding="utf-8")
        reviewed = run(root, "write-review", *common, "--checkpoint", "CP-001-WP-0", "--review", "review-input.json")
