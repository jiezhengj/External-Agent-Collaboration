...
7. Write a concise handoff into `.ai-collaboration/handoffs/`. For every `execute`, also write an expected-outcomes JSON file using [references/expected-outcomes.md](references/expected-outcomes.md). For a persistent topic, pass a short `--topic-goal` and `--stop-rule`; they create/update one local topic-state page. For multi-run Goal aggregation, also pass `--goal-contract`; do not save a transcript. Include only the objective, relevant background, allowed paths, prohibited paths, validation commands, expected output, and uncertainty. Never copy the complete Codex chat.
8. Keep Run and Goal judgments separate: Run `completed` means only that this attempt passed its outcomes, scope checks, and validation; it does not mean a persistent Goal is `achieved`. Pass `--goal-contract` to enable contract validation and cross-Run aggregation; use `goal_lifecycle.py` for explicit human acceptance, review, blocking, unblocking, and cancellation. Never infer Goal closure from free-text stop rules or topic state alone.
9. Do not include `.env` contents, tokens, credentials, private keys, customer exports, or unrelated private files.
...
Use `scripts/run_regression.py` as the repository-wide local gate: invoke it with `python3` on macOS/Linux or `py -3` on Windows. The same entry point is executed by the repository's macOS/Windows CI matrix.
...
The runner must not pass `--model`. Provider-internal default and FABLE/OPUS/SONNET/HAIKU/SUBAGENT mappings stay in the isolated CC Switch/Claude Code profile. For an auto-selected new topic, load the non-secret top-level `routing` policy from the shared/local provider documents: missing routing preserves fair rotation; `fair_round_robin`, `fixed`, and deterministic `weighted_round_robin` are supported. Explicit provider/session precedence, trust, readiness, health, permissions, outcomes, and one availability-only fallback remain unchanged. Task, response-contract, outcome, validation, or scope failure is not an availability failure.
