...
这个仓库用于说明并承载一个项目级协作流程：由 Codex 协调持续的外部编码协作者，预期接入 MiMo 和 DeepSeek，并通过本地 Claude Code CLI 调用。Codex 始终面向用户：接收请求、判断是否值得委派、检查结果并反馈。外部模型只是在明确边界内参与的协作者。

项目想解决的问题很具体：长期本地工作不能只靠一次性调用外部模型。需要把 provider 和会话分开、保留项目背景、限制文件修改范围，并验证实际发生了什么。

这里不预设某个模型总是更好。provider 的选择应来自同类任务的本地证据。

## 具体需求

1. **统一入口。** 用户只与 Codex 协作。只有在大规模本地上下文、受限实施或独立审查确有价值时，才引入外部协作者。
